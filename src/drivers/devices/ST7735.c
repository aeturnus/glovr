...
void ST7735_DrawCharS(int16_t x, int16_t y, char c, int16_t textColor, int16_t bgColor, uint8_t size){
  ST7735_DrawCharMap(x, y, (const char *)&Font[c*5], textColor, bgColor, size);
  /*
...
void ST7735_DrawCharMap(int16_t x, int16_t y, const char* map, int16_t textColor, int16_t bgColor, uint8_t size){
  uint8_t line; // vertical column of pixels of character in font
  int32_t i, j;
  if((x >= _width)            || // Clip right
     (y >= _height)           || // Clip bottom
...
void ST7735_DrawLine(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t color) {
  ...
  dy <<= 1; // dy is now 2*dy
  dx <<= 1; // dx is now 2*dx
  ST7735_DrawPixel(x0,y0,color); // Draw the first pixel.
